<template>
  <div class="DataSetting">
    <a-form v-if="data">
      <a-form-item :label="$t('Data Source')" name="dataOrgion" :rules="null">
        <a-select v-model:value="dataOrgion" style="width: 100%">
          <a-select-option value="static">{{ $t('Static Data') }}</a-select-option>
          <a-select-option value="api">{{ $t('Interface data') }}</a-select-option>
        </a-select>
      </a-form-item>
      <a-form-item v-if="dataOrgion === 'api'" :label="$t('Interface address')" name="api" :rules="null">
        <a-input-search v-model:value="total.inerfaceurl" :enter-button="$t('test')" @search="onTest(total.inerfaceurl)">
        </a-input-search>
      </a-form-item>
