...
export default {
  ...
  props: {
    ...
    z: {
      type: [String, Number],
      default: 'auto',
      validator: (val: number | string) => (typeof val === 'string' ? val === 'auto' : val >= 0)
    },
    handles: {
      type: Array,
    ...
    // 对应父组件pointerEvents为none的情况,这种情况下,取消点击的事件直接绑定在body上
  ...
  mounted: function () {
    ...
    if (this.parentElementCannotClick) {
  ...
  methods: {
      ...
      if (!this.$el.contains(target) && !regex.test(target.className)) {
        if (this.enabled && !this.preventDeactivation) {
          if (this.active === undefined) {
            ...
            $emit(this, 'update:active')
          }
          $emit(this, 'deactivated', target)
        }
    ...
    changeWidth(val) {
      ...
      let right = restrictToBounds(this.parentWidth - newWidth - this.left, this.bounds.minRight, this.bounds.maxRight)
      let bottom = this.bottom
      if (this.lockAspectRatio) {
    ...
    changeHeight(val) {
      ...
      let bottom = restrictToBounds(this.parentHeight - newHeight - this.top, this.bounds.minBottom, this.bounds.maxBottom)
      let right = this.right
      if (this.lockAspectRatio) {
  ...
  computed: {
    handleStyle() {
      return (stick) => {
        ...
        const size: any = (this.handleInfo.size / this.scaleRatio).toFixed(2)
  ...
  emits: ['contextmenu', 'update:active', 'dragging', 'resizing', 'refLineParams', 'resizestop', 'dragstop', 'activated', 'deactivated']
}
</script>
